...
nuxt-easy-texts is a Nuxt 3 module for key-based text/translation management. It
extracts translatable text keys from source code at build time and loads
translations at runtime from a user-defined loader.
...
The module scans source files for `$texts()` and `$textsPlural()` calls,
extracting keys and default texts:

1. **Collector** (`src/build/classes/Collector.ts`) - Manages file collection
   and extraction state
2. **CollectedFile** (`src/build/classes/CollectedFile.ts`) - Parses individual
   files for text calls
3. **ModuleHelper** (`src/build/classes/ModuleHelper.ts`) - Nuxt integration
   utilities
4. **DevModeHandler** (`src/build/classes/DevModeHandler.ts`) - Hot reload
   support in dev mode
...
- `$textsPlural('key', count, 'One', '@count items')` →
  `$textsPlural('key', count)`

This ensures builds contain only keys, requiring runtime loading.

### Runtime

- **Plugin** (`src/runtime/plugins/texts.ts`) - Initializes loader, provides
  `$texts`/`$textsPlural` globally
- **Composable** (`src/runtime/composables/useEasyTexts.ts`) - Alternative
  access supporting language overrides
- **Loader** (`src/loader.ts`) - User defines `~/app/easyTexts.loader.ts` to
  load translations
...
- **Extraction**: A parsed `$texts`/`$textsPlural` call with key, context, and
  default text
- **Context**: Optional namespace prefix in keys (e.g., `cart.addButton` has
  context `cart`)
- **TextsState**: Runtime map of keys to loaded translations
- **Language Override**: `<EasyTextsLanguageOverride>` component for partial
  page translation
